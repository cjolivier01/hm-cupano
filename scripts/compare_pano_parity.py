...
def run_python_stitch(directory: Path, num_images: int, levels: int, adjust: bool, device: torch.device, output_path: Path) -> torch.Tensor:
...
def main() -> int:
    ...
    try:
        ...
        py_out = work_dir / f"python_levels{args.levels}_n{num_images}.png"
        diff_out = work_dir / f"diff_levels{args.levels}_n{num_images}.png"
        metrics_out = work_dir / f"metrics_levels{args.levels}_n{num_images}.json"

        device = torch.device(args.device)
        run_python_stitch(work_dir, num_images, args.levels, args.adjust, device, py_out)
