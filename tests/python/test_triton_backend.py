...
from cupano.ops import copy_roi, remap_to_canvas, remap_to_canvas_with_dest_map
...
def test_copy_roi_triton_matches_torch(gpu_device: torch.device) -> None:
    src = torch.arange(1 * 16 * 16 * 4, device=gpu_device, dtype=torch.float32).reshape(1, 16, 16, 4).contiguous()
    dest_torch = torch.zeros((1, 20, 20, 4), device=gpu_device, dtype=torch.uint8)
    dest_triton = dest_torch.clone()

    copy_roi(src, dest_torch, Rect(0, 0, 8, 7), 3, 4, 5, 6, backend="torch")
    copy_roi(src, dest_triton, Rect(0, 0, 8, 7), 3, 4, 5, 6, backend="triton")
    assert_equal(dest_torch, dest_triton)


def test_copy_roi_negative_src_offset_matches_torch(gpu_device: torch.device) -> None:
    src = patterned_image(16, 12, 0, gpu_device)
    dest_torch = torch.zeros((1, 12, 20, 4), device=gpu_device, dtype=torch.uint8)
    dest_triton = dest_torch.clone()

    copy_roi(src, dest_torch, Rect(0, 0, 14, 8), -5, 2, 3, 1, backend="torch")
    copy_roi(src, dest_triton, Rect(0, 0, 14, 8), -5, 2, 3, 1, backend="triton")
    assert_equal(dest_torch, dest_triton)


def test_remap_triton_matches_torch(gpu_device: torch.device) -> None:
    src = patterned_image(32, 24, 0, gpu_device)
    dest_torch = torch.zeros((1, 32, 40, 4), device=gpu_device, dtype=torch.uint8)
    dest_triton = dest_torch.clone()
    ...
    remap_to_canvas(src, dest_torch, map_x, map_y, 4, 3, backend="torch")
    remap_to_canvas(src, dest_triton, map_x, map_y, 4, 3, backend="triton")
    assert_equal(dest_torch, dest_triton)


def test_remap_with_dest_map_triton_matches_torch(gpu_device: torch.device) -> None:
    src = patterned_image(24, 24, 1, gpu_device)
    dest_torch = torch.zeros((1, 24, 24, 4), device=gpu_device, dtype=torch.uint8)
    dest_triton = dest_torch.clone()
    ...
    remap_to_canvas_with_dest_map(src, dest_torch, map_x, map_y, 0, seam, 0, 0, backend="torch")
    remap_to_canvas_with_dest_map(src, dest_triton, map_x, map_y, 0, seam, 0, 0, backend="triton")
    assert_equal(dest_torch, dest_triton)


def test_cuda_pano_triton_matches_torch(gpu_device: torch.device) -> None:
    width = 96
    height = 32
    x2 = 48
    ...
    image1 = patterned_image(width, height, 0, gpu_device)
    image2 = patterned_image(width, height, 2, gpu_device)

    torch_pano = CudaStitchPano(1, 1, masks, quiet=True, backend="torch")
    triton_pano = CudaStitchPano(1, 1, masks, quiet=True, backend="triton")
    out_torch = torch_pano.process(image1, image2)
    out_triton = triton_pano.process(image1, image2)
    assert_equal(out_torch, out_triton)


def test_cuda_pano_n_triton_matches_torch(gpu_device: torch.device) -> None:
    width = 48
    height = 24
    ...
    inputs = [patterned_image(width, height, i, gpu_device) for i in range(3)]

    torch_pano = CudaStitchPanoN(1, 0, masks, quiet=True, backend="torch")
    triton_pano = CudaStitchPanoN(1, 0, masks, quiet=True, backend="triton")
    out_torch = torch_pano.process(inputs)
    out_triton = triton_pano.process(inputs)
    assert_equal(out_torch, out_triton)
