...
__global__ void BatchedRemapKernelEx(
  ...
  if (srcX < srcW && srcY < srcH) {
    ...
    destImage[destIdx] = remap_default<T_out>(deflt);
  }
}
...
__global__ void BatchedRemapKernelExOffset(
  ...
  if (srcX < src.width && srcY < src.height) {
    ...
    *dest_pos = perform_cast<T_out>(*src_pos);
    ...
    if (!no_unmapped_write || srcX != kUnmappedPositionValue) {
      *dest_pos = remap_default<T_out>(deflt);
      if constexpr (sizeof(T_out) / sizeof(BaseScalar_t<T_out>) == 4) {
...
__global__ void BatchedRemapKernelExOffsetWithDestMap(
  ...
  if (dest_image_map[checkIdx] == this_image_index) {
    ...
    if (srcX < src.width && srcY < src.height) {
      ...
      *surface_ptr(dest, b, destX, destY) = remap_default<T_out>(deflt);
    }
  }
}
...
__global__ void BatchedRemapHardSeamKernelN(
  ...
  if (srcX < static_cast<int>(src.width) && srcY < static_cast<int>(src.height)) {
    *surface_ptr(dest, b, x, y) = perform_cast<T_out>(*surface_ptr(src, b, srcX, srcY));
  }
}
...
cudaError_t batched_remap_hard_seam_kernel_n(
  ...
  dim3 gridDim(
      ...
      batchSize);
  BatchedRemapHardSeamKernelN<T, T_out><<<gridDim, blockDim, 0, stream>>>(
...
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar3, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar3, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar3, uchar3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(float1, float1)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(float3, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(float3, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(float4, float3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(float4, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar4, float3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar4, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(uchar4, half3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(half4, half3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(half4, half4)
...
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar3, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar3, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar3, uchar3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(float1, float1)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(float3, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(float3, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(float4, float3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(float4, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar4, float3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar4, float4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(uchar4, half3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(half4, half3)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(half4, half4)
...
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_WITH_DEST_MAP(half4, half4)
...
INSTANTIATE_BATCHED_REMAP_HARD_SEAM_N(half4)

INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET(Rgb10A2, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_ROI(Rgb10A2, half4)
INSTANTIATE_BATCHED_REMAP_KERNEL_EX_OFFSET_WITH_DEST_MAP(Rgb10A2, half4)
template cudaError_t batched_remap_hard_seam_kernel_n<Rgb10A2, half4>(
    const CudaSurface<Rgb10A2>*,
    const unsigned short* const*,
    const unsigned short* const*,
    const int2*,
    const int2*,
