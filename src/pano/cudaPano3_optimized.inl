...
CudaStatusOr<std::unique_ptr<CudaMat<T_pipeline>>> 
CudaStitchPano3<T_pipeline, T_compute>::process_impl_optimized(
  ...
  if (!stitch_context.is_hard_seam()) {
    ...
    cuerr = cudaMemsetAsync(stitch_context.cudaFull0->data(), 0, 
                           stitch_context.cudaFull0->size(), stream);
    CUDA_RETURN_IF_ERROR(cuerr);
    cuerr = cudaMemsetAsync(stitch_context.cudaFull1->data(), 0, 
                           stitch_context.cudaFull1->size(), stream);
    CUDA_RETURN_IF_ERROR(cuerr);
    cuerr = cudaMemsetAsync(stitch_context.cudaFull2->data(), 0, 
                           stitch_context.cudaFull2->size(), stream);
    CUDA_RETURN_IF_ERROR(cuerr);
...
template <typename T_pipeline, typename T_compute>
CudaStatusOr<std::unique_ptr<CudaMat<T_pipeline>>> 
CudaStitchPano3<T_pipeline, T_compute>::process_optimized(
    const CudaMat<T_pipeline>& inputImage0,
    const CudaMat<T_pipeline>& inputImage1,
    const CudaMat<T_pipeline>& inputImage2,
    cudaStream_t stream,
