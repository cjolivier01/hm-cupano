...
struct StitchingContextN {
  ...
  std::unique_ptr<CudaMat<T_compute>> cudaBlendSoftSeam;     // N-channel mask
  ...
  bool is_hard_seam() const { return is_hard_seam_; }
...
class CudaStitchPanoN {
 public:
  CudaStitchPanoN(int batch_size, int num_levels, const ControlMasksN& control_masks, bool match_exposure = false, bool quiet = false);

  int canvas_width() const { return canvas_manager_->canvas_width(); }
  int canvas_height() const { return canvas_manager_->canvas_height(); }
  int batch_size() const { return stitch_context_->batch_size(); }
  const CudaStatus status() const { return status_; }
 ...
 private:
  ...
  CudaStatus blend_soft_dispatch(std::vector<const T_compute*>& d_ptrs, cudaStream_t stream);
